module CorpWechatsJournalsPatch
  ...
  def send_by_dingtalk(send_people_dd)
    ...
    begin
      ...
      issue_text = "#{@issue.tracker} ##{@issue.id}: #{@issue.subject} #{@issue.status} #{@issue.done_ratio}% #{l(:msg_by)} #{@issue.msg_author} #{l(:msg_updated)}"
      msg_avatar_url = @issue.msg_avatar_url('https')
